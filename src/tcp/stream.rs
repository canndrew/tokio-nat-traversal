    ...
    fn fmt(&self, f: &mut fmt::Formatter) -> fmt::Result {
        ...
        match *self {
            ...
            AllAttemptsFailed(ref attempt_errors, ref igd_error, ref map_error) => {
                ...
                write!(f, "All {} connection attempts failed with errors: {:#?}",
                           attempt_errors.len(), attempt_errors)?;
            },
        }
        Ok(())
    }
...
impl TcpStreamExt for TcpStream {
    ...
    ) -> BoxFuture<TcpStream, ConnectReusableError>
    {
        let try = || {
            let builder = {
                TcpBuilder::bind_reusable(&bind_addr)
                .map_err(ConnectReusableError::Bind)?
            };
